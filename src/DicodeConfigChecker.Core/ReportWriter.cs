...
{
    public static async Task<IReadOnlyList<string>> WriteAsync(string directory, RunSummary summary, CancellationToken cancellationToken = default)
    {
        Directory.CreateDirectory(directory);
        var alive = summary.Results.Where(x => x.IsAlive).ToList();
        var configs = alive.Where(x => !x.Candidate.IsTelegramProxy).Select(x => x.Candidate.Raw).ToArray();
        var proxies = alive.Where(x => x.Candidate.IsTelegramProxy).Select(x => x.Candidate.Raw).ToArray();
